    ...
    for k, vals in series.items():
        ...
        for div in allowed_divisors_voltage:
            ...
            if _in_range(vv, 80.0, 300.0):
                target = 120.0 if abs(vv - 120.0) < abs(vv - 230.0) else 230.0
                closeness = 1.0 - min(_rel_err(vv, target), 1.0)
...
class TuyaCollector:
    def __init__(
        ...
        self.autodiscover_probe_dps = [str(x) for x in autodiscover_probe_dps]
    ...
    def _wanted_dps_list(self, st: TuyaDeviceState) -> List[str]:
        if st.discovery_ready and st.dps_current and st.dps_power and st.dps_voltage:
            ...
            keys = ["1", "18", "19", "20"] + self.autodiscover_probe_dps[:]
    ...
    def _try_known_layout_18_19_20(self, st: TuyaDeviceState, dps: Dict[str, Any]) -> bool:
        rc = _to_float(dps.get("18"))
        rp = _to_float(dps.get("19"))
        rv = _to_float(dps.get("20"))

        if rc is None or rp is None or rv is None:
            return False

        current = float(rc) / 1000.0
        power = float(rp) / 10.0
        voltage = float(rv) / 10.0

        if not (
            _in_range(voltage, 60.0, 320.0)
            and _in_range(current, 0.0, 80.0)
            and _in_range(power, 0.0, 20000.0)
        ):
            return False
        ...
        st.dps_power = "19"
        st.dps_voltage = "20"
        st.scale_current = 1000.0
        st.scale_power = 10.0
        st.scale_voltage = 10.0

        st.discovery_ready = True
        st.discovery_pending = False
        st.discovery_confidence = 0.999
        st.discovery_reason = "known_schema_18_19_20"
        st.needs_updatedps = True

        if not st.dps_relay and _parse_relay(dps.get("1")) is not None:
            st.dps_relay = "1"
            st.relay_ready = True
            st.relay_confidence = 0.999
            st.relay_reason = "known_schema"
    ...
    def _maybe_autodiscover(self, st: TuyaDeviceState, payload: ScrapePayload) -> None:
        ...
        if not st.discovery_ready:
            self._try_known_layout_18_19_20(st, payload.dps)
        ...
        if not st.discovery_ready:
            inferred = infer_layout_level2(
                samples=st.samples,
                allowed_divisors_voltage=[1.0, 10.0, 100.0, 1000.0],
                allowed_divisors_power=[1.0, 10.0, 100.0, 1000.0, 10000.0],
                allowed_divisors_current=[1.0, 10.0, 100.0, 1000.0, 10000.0],
    ...
    def collect(self):
        ...
        build = GaugeMetricFamily("tuya_smart_plug_exporter_build_info", "Exporter build information.", labels=["version", "python"])
...
def main() -> int:
    ...
    cfg_path = args.config_file or os.environ.get("TUYA_SMART_PLUG_EXPORTER_CONFIG", "").strip() or None
    ...
    listen_default = str(web_cfg.get("listen_address", "0.0.0.0:9122"))
